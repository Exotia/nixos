...
function GetEntries()
  local entries = {}
  local user_theme_dir = os.getenv("HOME") .. "/.config/arch/themes"
  local arch_path = os.getenv("OMARCHY_PATH") or ""
  local default_theme_dir = arch_path .. "/themes"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if preview_path and preview_path ~= "" then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "nix-theme-set " .. theme_name,
            },
          })
        end
      end
